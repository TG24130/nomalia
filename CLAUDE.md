...
```json
{
  ...
  "tourisme": { "type": "trip-liberte", "lieuxRetenus": [] },
...
- `tourisme.type` : `business`, `trip-liberte`, `safari`, `romantique`, `repos-total`

### Collection `fiches/{ficheId}` (cache)

Contient le JSON de la fiche en 10 points (§6), `genereLe`, `expireLe`.

### Collections `lieux/{cleCache}`, `randos/{cleCache}` et `conseilsMois/{cleCache}` (cache)
...
Champ optionnel « prix total trouvé pour vos N nuits » (`hebergement.prixTotal`), le montant qu'affichent les sites, pour affiner le budget.
