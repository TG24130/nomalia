...
import { consigneLangue, nomPays } from './langues.js';

/** Consigne permanente. */
export const SYSTEME_CONSEIL_MOIS = `Tu es un assistant de préparation de voyage. Tu conseilles le mois de départ vers une destination, pour un voyageur qui part de son pays, indiqué dans la demande.
...
- Si la destination est ambiguë, retiens son sens le plus courant pour un voyageur de ce pays.
 ...
 * @param {{ destination: string, langue: string, nationalite: string }} parametres
 * @returns {string}
 */
export function promptConseilMois({ destination, langue, nationalite }) {
  return `Destination : ${destination}
Pays du voyageur : ${nomPays(nationalite)}
